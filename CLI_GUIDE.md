...
```dart
class ProductPresenter extends Presenter<ProductViewController> {
  final GetProductUseCase _getProduct;
  final GetProductListUseCase _getProductList;

  Future<void> loadProducts() async {
    // Set specific loading state
    _state = _state.copyWith(isGettingList: true);

    final result = await execute(_getProductList, const NoParams());

    result.fold(
      (products) => _state = _state.copyWith(
        isGettingList: false,
